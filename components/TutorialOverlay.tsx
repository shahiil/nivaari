...
export default function TutorialOverlay({ onFinish }: TutorialOverlayProps) {
  ...
  const modalStyles = 'absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4';
  const hintStyles = 'absolute inset-x-0 top-6 z-50 flex justify-center p-4 pointer-events-none';
  const boxStyles = 'bg-white rounded-lg p-6 max-w-md text-center pointer-events-auto shadow-2xl';
      ...
      <div className={modalStyles}>
        <div className={boxStyles}>
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <button className="mt-4 inline-block px-4 py-2 bg-slate-600 text-white rounded hover:bg-slate-700" onClick={close}>Skip</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className={hintStyles}>
        <div className={boxStyles}>
          ...
          <p className="mb-6">Choose any tool from the toolbar and tap on the grid to add a tile. The map stays usable while this hint is open.</p>
          <p className="text-sm text-gray-500">(You can drag for multi‑tile shapes, too)</p>
          <button className="mt-4 inline-block px-4 py-2 bg-slate-600 text-white rounded hover:bg-slate-700 pointer-events-auto" onClick={close}>Skip Tutorial</button>
        </div>
      </div>
    );
  }
